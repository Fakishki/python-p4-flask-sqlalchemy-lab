...
def zookeeper_by_id(id):
    ...
    if not animals:
        response_body += f'<ul>{zookeeper.name} has no animals they care for</ul>'
...
def enclosure_by_id(id):
    ...
    if not animals:
        response_body += f'<ul>Enclosure {enclosure.id} ({enclosure.environment}) has no animals in it</ul>'
